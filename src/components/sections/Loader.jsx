...
export default function Loader({ onComplete }) {
  ...
  useEffect(() => {
    ...
    // AK entrance
    tl.from(akRef.current, { scale: 1.05, opacity: 0, duration: 0.8, ease: 'power2.out' })
    // Line expand
    ...
    // Exit
    tl.to(akRef.current, { scale: 1.15, opacity: 0, duration: 0.6 }, '+=0.1')
  ...
  return (
    ...
    >
      <div ref={akRef} className="text-center">
        <span
          className="font-display font-bold text-text block"
        ...
        <div
          ref={lineRef}
          className="h-[1.5px] bg-accent mx-auto mt-4"
          style={{ width: '120px' }}
        />
      </div>
      <span
        ref={counterRef}
        className="font-mono text-muted absolute bottom-8 right-8"
